...
class Emulator(Object):
    ...
    def __init__(
        ...
        if type(dataset) == str and type(model) == str:
            ...
            self.parameter_constriants = self.dataset.constraints.get("parameters", [])
            self.aux_param_space = getattr(self.dataset, "aux_param_space", None)
...
def load_emulator(emulator_folder):
    ...
    # Old pickles may lack 'task' on the Dataset; default to 'regression'.
    try:
        _task = emulator_to_load.task
    except AttributeError:
        emulator_to_load.dataset.task = "regression"
        _task = "regression"
