#!/bin/bash
# Ackley-5D scale sweep eval; GP and TabPFN run concurrently, each with internal workers.
set -euo pipefail
...
MODEL_PATH="${MODEL_PATH:-${EXP_DIR}/models/${TAG}/ppo_best.pt}"
...
run_eval() {
  local SURROGATE="$1"
  local SAVE_DIR="${EXP_DIR}/results/${TAG}_${SURROGATE}"
  echo "=== Ackley-5D eval (${TAG}): ${SURROGATE} ==="
