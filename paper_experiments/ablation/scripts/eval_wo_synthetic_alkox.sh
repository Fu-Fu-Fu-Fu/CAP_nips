...
#
# Evaluates a model trained with bnn_rff_alpha=0 (BNN mean only, no RFF diversity).
# Model must be trained first and placed at the MODEL_PATH below.
#
# Usage:
#   SURROGATE=gp bash paper_experiments/ablation/scripts/eval_wo_synthetic_alkox.sh
# =============================================================================
set -euo pipefail
...
# Model trained with bnn_rff_alpha=0 (no RFF diversity)
MODEL_PATH="paper_experiments/ablation/component/alkox/wo_synthetic/model/ppo_best.pt"
TAF_DATA="./data/taf_source_data_alkox_emulator_k10_transform.pkl"
SAVE_DIR="paper_experiments/ablation/component/alkox/wo_synthetic/results_${SURROGATE}"

echo "=== Ablation 2: w/o Synthetic Objective — Alkox (${SURROGATE}) ==="

if [[ ! -f "${MODEL_PATH}" ]]; then
    echo "ERROR: Model not found at ${MODEL_PATH}"
    echo "Please train the ablation model first (bnn_rff_alpha=0)."
    exit 1
fi
