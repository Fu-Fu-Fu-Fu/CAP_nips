#!/bin/bash
# =============================================================================
# Evaluate Alkox Default v2 (new training recipe, all components)
#
# Runs two surrogates (gp, tabpfn_base) in parallel.
#
# Usage:
#   bash paper_experiments/ablation/scripts/eval_default_v2_alkox.sh
# =============================================================================
set -euo pipefail
...
if [[ ! -f "${MODEL_PATH}" ]]; then
    ...
    echo "Please run train_default_v2_alkox.sh first."
    exit 1
fi
...
run_eval() {
    ...
    echo "=== Eval Alkox Default v2 (${SURROGATE}) ==="
...
# Run gp and tabpfn_base in parallel
run_eval "gp" &
PID_GP=$!
