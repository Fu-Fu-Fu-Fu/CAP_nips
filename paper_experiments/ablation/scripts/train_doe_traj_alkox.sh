#!/bin/bash
# =============================================================================
# Trajectory Ablation: DOE prior trajectories — Alkox
#
# Trains one DOE prior-data variant per invocation.
# Training parallelism is handled inside train_rl.py via --n_workers.
#
# Usage:
#   bash paper_experiments/ablation/scripts/train_doe_traj_alkox.sh doe_static
#   bash paper_experiments/ablation/scripts/train_doe_traj_alkox.sh doe_seq_local
# =============================================================================
set -euo pipefail
...
BASE_DIR="paper_experiments/ablation/trajectory/alkox"
METHOD="${1:-${DOE_METHOD:-}}"
...
run_train() {
    ...
    case "${METHOD}" in
        doe_static)
            ...
            BNN_PARAMS="${DATA_DIR}/bnn_surrogates_${TASK}_doe_static_k${K_VARIANTS}_kl0.001.npz"
            ;;
        doe_seq_local)
            ...
            BNN_PARAMS="${DATA_DIR}/bnn_surrogates_${TASK}_doe_seq_local_k${K_VARIANTS}_kl0.001.npz"
            DOE_ARGS=(--init_global 10 --batch_size 4 --radius0 0.20 --radius_min 0.05 --decay 0.90 --global_frac 0.40)
            ;;
        *)
...
"
    ...
    if [[ -f "${BNN_PARAMS}" ]]; then
        echo "[${METHOD}] SKIP BNN: ${BNN_PARAMS}"
    else
