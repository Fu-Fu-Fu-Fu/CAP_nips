#!/bin/bash
# =============================================================================
# Eval Trajectory Ablation: DOE — Alkox
#
...
#
# Usage:
#   bash paper_experiments/ablation/scripts/eval_doe_traj_alkox.sh doe_static
#   bash paper_experiments/ablation/scripts/eval_doe_traj_alkox.sh doe_seq_local
# =============================================================================
set -euo pipefail
...
taf_path() {
    case "$1" in
        doe_static) echo "${BASE_DIR}/doe_static/data/taf_source_data_alkox_emulator_doe_static_k10.pkl" ;;
        doe_seq_local) echo "${BASE_DIR}/doe_seq_local/data/taf_source_data_alkox_emulator_doe_seq_local_k10.pkl" ;;
...
run_eval() {
    ...
    echo "=== Eval Alkox ${METHOD} (${SURROGATE}) ==="
...
echo "Waiting for parallel Alkox ${METHOD} eval jobs..."
set +e
FAIL=0
for i in "${!PIDS[@]}"; do
    wait "${PIDS[$i]}"
