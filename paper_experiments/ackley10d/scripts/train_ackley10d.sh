#!/bin/bash
# Ackley-10D family training: oracle GP trajectory reconstruction + multiscale RFF.
set -euo pipefail
...
N_INIT="${N_INIT:-2}"
MAX_STEPS="${MAX_STEPS:-78}"
TOTAL_EVALS=$((N_INIT + MAX_STEPS))

TOTAL_EPISODES="${TOTAL_EPISODES:-5000}"
N_WORKERS="${N_WORKERS:-8}"
RUN_DIR="${RUN_DIR:-${EXP_DIR}/runs/ppo_${TASK}_msrff_mid_ep${TOTAL_EPISODES}}"
MODEL_DIR="${MODEL_DIR:-${EXP_DIR}/models/msrff_mid}"
...
RFF_LENGTH_SCALES="${RFF_LENGTH_SCALES:-0.08,0.16,0.32}"
RFF_ALPHAS="${RFF_ALPHAS:-0.20,0.30,0.45}"

mkdir -p "${RUN_DIR}" "${MODEL_DIR}" "${EXP_DIR}/logs" "${MPLCONFIGDIR}"

echo "=== Ackley-10D training ==="
