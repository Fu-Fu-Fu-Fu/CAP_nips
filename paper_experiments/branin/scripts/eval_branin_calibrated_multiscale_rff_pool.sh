#!/bin/bash
# =============================================================================
# Eval Branin calibrated multi-scale RFF + pool tuning (gp + tabpfn parallel)
#
# Usage:
...
MODEL_PATH="paper_experiments/branin/models/calibrated_multiscale_rff_pool/ppo_best.pt"
TAF_DATA="data/taf_source_data_branin_family_k10.pkl"
...
if [[ ! -f "${MODEL_PATH}" ]]; then
    ...
    echo "Run train_branin_calibrated_multiscale_rff_pool.sh first."
    exit 1
fi
...
run_eval() {
    local SURROGATE="$1"
    local SAVE_DIR="paper_experiments/branin/results/calibrated_multiscale_rff_pool_${SURROGATE}"

    echo "=== Branin calibrated multiscale RFF + pool eval (${SURROGATE}) ==="
    echo "Save: ${SAVE_DIR}"
...
echo "  Results: paper_experiments/branin/results/calibrated_multiscale_rff_pool_*/"
echo "================================================================"
