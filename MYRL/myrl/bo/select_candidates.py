"""
Train CAP-PPO: 使用 TabPFN 做贝叶斯优化的候选点选择
...
# ==================== Y 归一化 TabPFN 预测 ====================
def predict_tabpfn_with_normalization(regressor, X_ctx, y_ctx, X_test):
    """Y 值归一化的 TabPFN 预测，返回 raw 空间的 mean 和 std"""
    y_ctx = np.asarray(y_ctx, dtype=np.float32).reshape(-1)
    y_mean = float(y_ctx.mean())
    y_std = float(max(y_ctx.std(), 1e-8))
    y_ctx_norm = ((y_ctx - y_mean) / y_std).astype(np.float32)

    regressor.fit(np.asarray(X_ctx, dtype=np.float32), y_ctx_norm)
    full_out = regressor.predict(np.asarray(X_test, dtype=np.float32), output_type="full")

    mean_norm = np.asarray(full_out["mean"], dtype=np.float64)
    std_norm = np.asarray(compute_std_from_tabpfn_output(full_out), dtype=np.float64)

    # 反归一化回 raw 空间
    mean_raw = mean_norm * y_std + y_mean
    std_raw = std_norm * y_std

    return mean_raw, std_raw, full_out
...
def select_candidates(
    ...
    # 带 Y 归一化的预测
    y_pred_mean, y_pred_std, full_out = predict_tabpfn_with_normalization(
