...
from .goldstein_price import goldstein_price_numpy
...
def goldstein_price_family_numpy(X: np.ndarray, variant_params: Dict[str, float]) -> np.ndarray:
    """
    A simple Goldstein-Price variant family using the same transformation schema as branin_family:
    rotation (around center) -> axis scaling -> translation.
    """
    ...
    y = goldstein_price_numpy(np.stack([x1t, x2t], axis=1))
    y = alpha * y + beta
    return np.asarray(y, dtype=np.float32).reshape(-1)


_VARIANT_SUITE_SPECS: Dict[str, Dict[str, List[Tuple[float, float]]]] = {
    # Ranges are chosen to keep most transformed inputs near the original domain [-2, 2]^2.
    "in_range": {
        "dx": [(-0.1, 0.1)],
        "rotation": [(-15.0, 15.0)],
        "sx": [(0.9, 1.1)],
...
class GoldsteinPriceFamilyTask(TaskSpec):
    @property
    def task_name(self) -> str:
        return "goldstein_price_family"
    ...
    def evaluate_numpy(self, X: np.ndarray, variant_params: Optional[Dict[str, float]] = None) -> np.ndarray:
        return goldstein_price_family_numpy(X, variant_params or {})
    ...
    def estimate_global_min(self, variant_params: Optional[Dict[str, float]] = None, *, grid_size: int = 120) -> float:
        """
        Robust global-min estimator for the transformed family: coarse grid + multi-start L-BFGS-B.
        """
        ...
        y_grid = self.evaluate_numpy(grid_points, variant_params).reshape(-1)
        ...
        def func(x):
            ...
            return float(self.evaluate_numpy(x, variant_params).reshape(-1)[0])
